...
1. 说一下MVCC是什么?
2. 间隙锁的话解决了什么问题，和MVCC有冲突吗？
3. RR和RC级别下面是，事务里面的ReadView视图有什么区别呢？
4. 在普通的select（不加锁的读）为什么RR可以解决幻读，而RC不能解决幻读呢？以ReadView作为出发点进行思考？
5. 你是怎么使用RabbitMQ保证的分布式事务
6. RabbitMQ保证分布式事物在项目的使用场景，为什么要用？不用行不行？
7. 分布式事务？
8. 事务的ACID与实现原理？
9. 数据库中的锁机制？
