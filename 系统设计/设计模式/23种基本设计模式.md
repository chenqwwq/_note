...
**SpringBoot 中对 BeanFactoryPostProcessor 的调用可能就是这个法则吧。**
...
类图如下：
 ...
 ![image-20200610235339938](assets/%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F.png)
...
而另外一种**懒汉模式**，则是在此基础上增加了**懒加载**的特性，只有在需要的时候才会调用。
...
为了保证懒汉模式的线程安全性，确保实例对象的唯一性，所以又有了**静态内部类，枚举以及耳熟能详的双重检查锁模式**。
...
**使用了 synchronized 来保证创建过程的线程安全，用 volatile 保证 INSTANCE 的可见性。**
...
在加载外部类的时候并不会直接加载内部类，只有在使用到内部类的相关属性的时候才会去进一步加载，也就是说只有在调用 getLazySingle 方法的时候，因为使用了 InstanceHolder 类才会去加载内部类，而静态变量也会在类加载的过程中完成初始化。
...
在任何创建大量重复对象的场景下，都可以尝试用原型模式替换。
 ...
 ![image-20200613184728715](assets/%E5%8E%9F%E5%9E%8B%E6%A8%A1%E5%BC%8F.png)
...
> 这里的工厂就非常适合使用单例模式实现，因为可以提供一个全局唯一的访问点。
...
比如一个 BufferReader 的创建过程，原有代码是这样的：
...
但问题也明显，就比如我现在需要一个 FileReader，那么就只能增加一个方法或者修改原有方法，这就和上面说的一样，它违反了开闭原则。





咳咳咳。。所以基于扩展性考虑引申出了工厂方法模式。
