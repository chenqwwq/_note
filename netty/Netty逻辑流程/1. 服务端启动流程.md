...
Netty是对JDK NIO的进一步封装和扩展，但是Netty服务端的启动流程是基本保持Nio的模式不变，两者可以相互对照。



以下是原生的Nio服务端启动代码的简单版本:
...
Netty的启动过程中不一定按照一样的顺序，但是这些基本的东西都不会落下。



**在EventLoopGroup的初始化流程中创建EventLoop的过程就会通过SelectorProvider生成一个Selector，**相当于提前生成了Channel，所以剩下的就是Channel的创建，初始化以及注册。
...
![image-20201025231848892](/home/chen/Pictures/image-20201025231848892.png)
...
初始化Bootstrap的逻辑并不复杂，以下是其中几个相对重要的方法。
...
一般来说，第一个Group用于响应请求，建立连接，可以采用单线程方式，后面的Group用于响应Channel的IO事件，如果只指定一个Group则是这个Group全包。

**EventLoop都需要提前创建，值得注意的是，比如NioEventLoop，在创建的时候就会通过SelectorProvider创建一个Selector对象。**



#### channel

该方法的作用是指定服务端本地Channel的类型。

比如切换UDP协议和TCP协议以及NIO，OIO的实现逻辑在该方法中切换参数就可以实现。
...
childHandler是指定对于子Channel的业务逻辑，这里是用户的业务逻辑主要指定的地方。

childHandler方法中可以多接受一个线程池的参数，该线程池就是用来执行该Handler的线程。



Bootstrap创建完成后，就可以借助Bootstrap快速的拉起一个应用服务端。
...
首先通过initAndRegister创建一个Channel，并通过ChannelFuture异步传出，需要异步传出的原因是因为，Channel注册到EventLoop的流程是异步实现的。

之后如果ChanelFuture创建完成就直接调用doBind0()完成真正的绑定，如果未完成则添加一个Listener延迟绑定。
...
next()方法就是从EventLoopGroup管理的EventLoop中选择一个，然后调用它的register()方法。

返回值是ChannelFutures类型就表明Channel的注册流程是异步的。

最终register会调用到AbstractChannel$AbstractUnsafe#register(EventLoop eventLoop, final ChannelPromise promise)
...
如果当前线程在EventLoop中就直接调用，否则通过EventLoop执行最终的注册流程，这里就体现了Netty的一个执行原则：**Channel相关的事件都由Channel绑定的EventLoop执行。**
...
doRegister()方法是个模板方法，真实的注册逻辑会延迟到子类实现，以下是在AbstractNioChannel的实现:
...
![image-20201024223926801](/home/chen/Pictures/image-20201024223926801.png)
...
![image-20201024224336525](/home/chen/Pictures/image-20201024224336525.png)
...
![image-20201024225006309](/home/chen/Pictures/image-20201024225006309.png)

不出意外的是还是借用unsafe完成相对比较底层的操作，以下是AbstractChannel$AbstractUnsafe的bind方法:

![image-20201024225432544](/home/chen/Pictures/image-20201024225432544.png)

源码中的doBind方法是个模板方法延迟到了子类实现，最终Channel的子类自然是NioServerSocketChannel。

![image-20201024225644065](/home/chen/Pictures/image-20201024225644065.png)
