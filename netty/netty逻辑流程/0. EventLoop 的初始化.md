...
Netty 中一般会定义两个不同作用的 EventLoopGroup，BossGroup 和 WorkerGroup，分别对应的 Reactor 模型中的前端接收线程组以及后端事件处理线程组。

BossGroup 负责监听 Selector 上的 Accept 事件，并创建 对应的客户端 Channel 分派给 WorkerGroup。

WorkerGroup 也就是事件处理线程组，WorkerGroup 中的每个 EventLoop 都会持有一个 Selector 对象，接收到 BossGroup 传递的客户端 Channel 之后，就会将其绑定到 Selector 上。
