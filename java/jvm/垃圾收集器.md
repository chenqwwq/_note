...
| -XX:MaxGCPauseMillis           | 期望的 G1 GC 停顿时间，GC 时会尽量靠近这个值      | 过小会导致频繁 GC |
...
CMS 将原先的 GC 划分阶段，只有在不得不 STW 的阶段才会进入 STW 的状态，从而减少整体的 STW 事件，提高应用整体的吞吐率。
...
相比于 ParNew 等，G1 和 CMS 往往具有更短的 STW 事件，可以有更高的吞吐量，但是因为并发的缘故也就会出现一些问题：
...
漏标是在 GC 过程中非常严重的错误，主要是导致**存活的对象没有被标记，而被作为垃圾对象回收**。

> GC 的标记大部分情况都是标记存活对象而回收无标记的对象。

在三色标记法中，标记过后出现一条从黑色节点出发的对象，就可能出现漏标的情况。
...
G1 和 CMS 都是并发的垃圾回收器，这里的并发是指用户线程（mutator）和 GC 线程（collector）同时运行，所以伴随着两者的并发可能会出现一些标记错误的情况。

例如下图的情况：

![strong-weak-tricolor-invariant](assets/2020-03-16-15843705141834-strong-weak-tricolor-invariant.png)

图片来源：[Go 语言垃圾收集器的实现](https://draveness.me/golang/docs/part3-runtime/ch07-memory/golang-garbage-collector/#%E5%A2%9E%E9%87%8F%E5%92%8C%E5%B9%B6%E5%8F%91)

以上情况就会出现漏标，D 最终会被当成垃圾对象清除，这是非常严重的问题。
...
**增量更新简单说就是将所有从黑色标记出发的引用都会被保存，在最终标记阶段以该黑色节点为根节点扫描。**

**原始快照是在删除灰色节点指向白色节点的引用时保存，最终标记阶段重新扫描。**
