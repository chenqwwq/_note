...
较为早期的原型协议，以单行 GET /mypage.html 作为全部的请求格式。
...
完善 HTTP 报文格式，每个报文增加版本信息，增加请求头/响应头的概念。
...
**允许底层 TCP 连接保活而重复使用**（connection: keep-alive。

增加管线技术，在收到前一个报文的响应之前就可以发出当前请求报文（一定程度上缓解队头阻塞问题，降低了通信延迟。

引入通过请求头完成的缓存机制。

引入响应的分块传输。

引入 C/S 之间的协商机制，包括响应体格式，编码等。
