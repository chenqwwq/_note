...
政采云有限公司是政府采购行业的头部企业，为全国十数个省份、直辖市政府提供采购相关解决方案。

作为业务中台-用户中心的成员，**维护用户中心、入驻中心、代理机构库、供应商库、消息代办、短信网关等系统**。

<br>

### 杭州学海教育有限公司（2020年4月 - 2022年6月）
...
作为业务中台消息代办、代理机构库的项目 Owner，带着两个应届生负责项目的日常维护。

#### 消息待办服务

公司内部的消息待办服务，消息日均200w、代办日均50w的调用量。

##### 项目经验

1. 对于 MQ 的消费优化，引入 Redis List 作为中间缓存，经由消息分片以及 ElasticJob 分片调度实现消费速度的三倍提升
2. 优化发送逻辑，添加消息的即时性等级参数，通过线程池的隔离保证重要消息的实时发送
3. 基础数据的多维度分表设计，基于用户id对基础消息表分片，基于待办id对待办的 hot 表进行分片
...
1. 钉钉/浙政钉单点登录逻辑设计和实现，用户可以直接从钉钉或者浙政钉登陆到浙里报账系统
2. 钉钉/浙政钉用户数据同步模块，实现全浙江以及部分国企的用户数据同步，基于抽象工厂模式实现机构、部门、用户的多类同步方法
3. 
...
#### 四库项目（入驻中心、代理机构库、供应商库

整个四库项目是公司用户中心的核心业务，负责各类用户的入驻，以及供应商、代理机构的合规性管理。

项目至今有供应商200w+，代理机构3w+，评审专家50w+的数据量，**以政采云平台为云端，各省市独立系统为岛端。**

##### 项目经验

1. 代理机构云岛同步功能，代理机构在岛端入驻时从云端拉取完整数据并留痕，也会同步最新数据到云端
2. 代理机构中采网对接，主导了一期的快速迭代到二期优化的所有方案设计，降低三方接口依赖
3. 云到岛的数据迁移，主导云平台到广西、军队采购、企业采购等环境的供应商和代理机构数据迁移
4. 负责 Apache Age 图数据库的调研，并最终落地实现供应商股权关系模块，对外输出二方 SDK 包提供其他业务组实现类似风险预警等功能
5. 代理机构日常考试模块，对接采云学院、信息报，实现考试创建、名单确认、名单增补、成绩录入等相关功能
